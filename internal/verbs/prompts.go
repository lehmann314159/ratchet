...
PART 2 — one spec-contradiction pass over EVERY test function and sub-test. The pre-pass cannot decide these; you must. Any "Other observations" the pre-pass listed (e.g. an asserted string it could not find in the spec) are starting points for this pass, not findings in themselves. Check:
(a) Assertion correctness — independently derive the expected value strictly from the spec's stated rule applied literally. Do not supplement it with outside domain convention, "how this is usually done", or unstated exceptions, even if the literal result feels unintuitive. If applying the literal rule genuinely requires an assumption the spec doesn't provide, report that gap as a finding rather than accepting or inventing a value.
(b) Over-specification — an assertion that pins an exact value, string, or format that NEITHER the bead spec NOR the design excerpts require. "Returns an error" / "Err is non-empty" / "responds non-200" does not license pinning an exact error string or rendered fragment. The ASSERTION is the defect; name the looser property actually required. Exception: values the design document pins verbatim (a pinned disassembly string, an exact 'runtime error: division by zero' message, a required class="error" token) must be asserted exactly.
(c) Satisfiability — trace the cumulative state at each assertion (every assignment, append, nil-out since the enclosing scope began). Given a correct implementation, can the assertion ever pass? When reasoning about an upstream function whose source is in the Implementation Files, verify against its literal content, not a guess.

You have a run_go_snippet tool (source): it compiles and runs a small stdlib-only Go program and returns its output. The pre-pass already ran the tests, so this is OPTIONAL — but before stating any claim about Go/stdlib runtime behavior in a finding (does this string survive this escaping function, do these two errors compare equal, how does this stdlib call format this input), verify it with a snippet rather than reasoning from general impressions.
