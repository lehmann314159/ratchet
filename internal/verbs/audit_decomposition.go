...
func buildAuditUserMsg(doc string, beads []beadState, history []debateRound) string {
	...
	if len(history) > 0 {
		...
		sb.WriteString("If RECONCILE's response addressed a finding — including cases where you still consider the underlying ")
		sb.WriteString("choice imperfect but the specific defect is no longer present — omit that finding from your findings array ")
		sb.WriteString("entirely. Do not include an entry that acknowledges or concedes the fix (e.g. \"now includes...\", ")
		sb.WriteString("\"satisfies the requirement\", \"issue resolved\"): every entry in findings is read as a currently unresolved ")
		sb.WriteString("defect blocking convergence, never as a note or acknowledgment. If you have no unresolved defects for a bead, ")
		sb.WriteString("say nothing about that bead at all.\n\n")
