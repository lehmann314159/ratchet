...
// fresh traces/bead-{id}-{kind}-{n}/ directory, preserving relative paths,
...
func SnapshotBeadFiles(folderPath, kind string, beadID int64, outputFiles []string) (dir string, preserved []string, err error) {
	...
	n := 1
	for {
		if _, err := os.Stat(filepath.Join(tracesDir, fmt.Sprintf("bead-%d-%s-%d", beadID, kind, n))); os.IsNotExist(err) {
			break
		}
		n++
	}
	snapshotDir := filepath.Join(tracesDir, fmt.Sprintf("bead-%d-%s-%d", beadID, kind, n))
