...
func TestParseRewindManifest(t *testing.T) {
	md := `# Bead 42 rewind snapshot
...
prefer Square{Row,Col}

Supersedes/retracts: Note 1
...
- game_test.go: deleted (test file, regenerated by REFINE_TESTS_WRITE)
- game.go: reset to scaffold stub
- helper.go: deleted (not in SURVEY manifest, no stub baseline)
`
	guidance, tests, stubbed, deleted := parseRewindManifest(md)
	if guidance != "use the recursive form\nprefer Square{Row,Col}" {
		t.Errorf("guidance = %q", guidance)
	}
	if len(tests) != 1 || tests[0] != "game_test.go" {
		t.Errorf("tests = %v", tests)
	}
	if len(stubbed) != 1 || stubbed[0] != "game.go" {
		t.Errorf("stubbed = %v", stubbed)
	}
	if len(deleted) != 1 || deleted[0] != "helper.go" {
		t.Errorf("deleted = %v", deleted)
	}

	guidance, _, _, _ = parseRewindManifest("## Guidance for next attempt\n\n(no new guidance note given)\n")
	if guidance != "" {
		t.Errorf("expected empty guidance, got %q", guidance)
	}
}

// TestBeadDetail_RendersRewindHistory drives a real rewind through the UI
// handler, then confirms the bead-detail page surfaces it as a Rewind History
// row (with the guidance note and files-reset summary) and drops a divider
// into the execution-history table.
func TestBeadDetail_RendersRewindHistory(t *testing.T) {
	s, d := openTestServer(t)
	pid := seedProject(t, d)
	...
	rec := doPost(t, s, "/beads/"+strconv.FormatInt(beadID, 10)+"/rewind",
		url.Values{"note": {"try the recursive form"}})
	if rec.Code != http.StatusSeeOther {
		t.Fatalf("rewind: expected 303, got %d: %s", rec.Code, rec.Body.String())
	}
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("bead detail: expected 200, got %d: %s", rec.Code, body)
	}
	for _, want := range []string{
		"Rewind History", "Rewind 1", "try the recursive form", "rewind-divider",
		"/beads/" + strconv.FormatInt(beadID, 10) + "/snapshot/1", // HasSnapshot: manifest was read + parsed
	} {
	...
	// The project page's beads table badges the rewound bead.
	proj := getBody(t, s, "/projects/"+strconv.FormatInt(pid, 10))
	if !strings.Contains(proj, "rewind-badge") || !strings.Contains(proj, "⟲ 1") {
		t.Errorf("project page missing rewind badge:\n%s", proj)
	}
}
